/**
 * COMPÁS — Generador HTML del Plan Local de Salud (documento completo)
 * core/documentos/generarHTMLPlanLocal.js
 *
 * Extraído de COMPAS.html (L.28866) el 2026-03-14
 *
 * Dependencias resueltas por parámetro (ctx) o fallback a window.*:
 *   - ctx.municipio              → window: getMunicipioActual()
 *   - ctx.planLocalSalud         → window.planLocalSalud
 *   - ctx.datosMunicipioActual   → window.datosMunicipioActual
 *   - ctx.accionesAgenda         → window.accionesAgenda
 *   - ctx.generarDeterminantes   → window: generarDeterminantesCompletosPDF()
 *   - ctx.generarParticipacion   → window: generarSeccionParticipacionCiudadanaPDF()
 *   - estudiosHtmlExterno        → window: document.getElementById(...)
 *
 * Dependencias residuales (fallbacks activos, ver notas al pie):
 *   - getMunicipioActual()                   — DOM, fallback en ctx.municipio
 *   - getNombreMunicipio()                   — extraída en core/utils/municipios.js
 *   - generarPortadaPLS()                    — extraída en core/documentos/cabeceraInstitucionalPDF.js
 *   - generarDeterminantesCompletosPDF()     — monolito, inyectable por ctx.generarDeterminantes
 *   - generarSeccionParticipacionCiudadanaPDF() — monolito, inyectable por ctx.generarParticipacion
 *   - generarConclusionesCompletasPDF()      — extraída en core/documentos/conclusionesPDF.js
 *   - generarAgendaCompletaPorEntorno()      — extraída en core/documentos/agendaCompletaPDF.js
 *   - generarCronogramaCompilado()           — extraída en core/documentos/agendaCompletaPDF.js
 *   - window.datosParticipacionCiudadana     — condición de participación
 *   - COMPAS_VERSION                         — con guard typeof
 *
 * Dependencias residuales inline (no importables aún):
 *   - getMunicipioActual()                      — DOM puro, solo fallback cuando ctx.municipio ausente
 *   - generarDeterminantesCompletosPDF()        — en monolito, solo fallback cuando ctx.generarDeterminantes ausente
 *   - generarSeccionParticipacionCiudadanaPDF() — en monolito, solo fallback cuando ctx.generarParticipacion ausente
 *   - window.datosParticipacionCiudadana        — condición de participación, permanece como window.*
 *   - COMPAS_VERSION                            — con guard typeof, no requiere import
 */

import { getNombreMunicipio }                              from '../utils/municipios.js';
import { generarPortadaPLS }                               from './cabeceraInstitucionalPDF.js';
import { generarConclusionesCompletasPDF }                 from './conclusionesPDF.js';
import { generarAgendaCompletaPorEntorno,
         generarCronogramaCompilado }                      from './agendaCompletaPDF.js';

// ── Export público: generarHTMLPlanLocal ──────────────────────────────────────

export function generarHTMLPlanLocal(completo, estudiosHtmlExterno = null, ctx = null) {

    const municipio = ctx?.municipio || getMunicipioActual();

    const nombre = getNombreMunicipio(municipio);

    const planLocalSalud       = ctx?.planLocalSalud       || window.planLocalSalud;
    const datosMunicipioActual = ctx?.datosMunicipioActual || window.datosMunicipioActual;
    const accionesAgenda       = ctx?.accionesAgenda       || window.accionesAgenda;
    ...
    html += (ctx?.generarDeterminantes || generarDeterminantesCompletosPDF)();
    ...
    var _estudiosHTMLPDF = estudiosHtmlExterno ?? (document.getElementById('seccion-estudios-complementarios')?.innerHTML.trim() || '');
    ...
    if (window.datosParticipacionCiudadana && typeof (ctx?.generarParticipacion || generarSeccionParticipacionCiudadanaPDF) === 'function') {

        html += (ctx?.generarParticipacion || generarSeccionParticipacionCiudadanaPDF)();
